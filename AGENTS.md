...
maven-enforcer plugin). Single Maven module, package `ch.dboeckli.guru.jpa.intro`. It demonstrates
JPA repositories (natural/UUID/composite/embedded IDs) against H2 (MySQL-compat mode) and MySQL,
with schema management via **Flyway** and **Liquibase**. App port `8080`.
...
- `mysql`: MySQL via Docker Compose — `compose-mysql-with-flyway.yaml` or
  `compose-mysql-with-liquibase.yaml`; schema via Flyway (`db/migration`) or Liquibase (`db/changelog`).
- IntelliJ run configs in `.run/`: `Spring6Application h2`, `Spring6Application mysql flyway`,
  `Spring6Application mysql liquibase`.
...
- H2 tests: `@DataJpaTest` + `@AutoConfigureTestDatabase(replace = NONE)` +
  `@Import(DataInitializer.class)` (H2 runs in MySQL-compat mode).
- MySQL ITs: `@DataJpaTest` + `@ActiveProfiles("test_mysql_with_flyway")` /
  `test_mysql_with_liquibase`; they need Docker (MySQL from the test profile).
- A custom `TestClassOrderer` sorts test classes; `LocaleExtension` forces `Locale.US`.

## Architecture

- `domain` (+ `domain/example/{natural,uuid,composite}`) entities; `repository` Spring Data JPA
  repositories; `bootstrap/DataInitializer` seeds data.
- Schema migrations: `src/main/resources/db/migration` (Flyway) and `db/changelog` (Liquibase).

## Deploy / CI

- Deployment is Helm-only: chart in `helm-charts/`, packaged to `target/helm/repo/`, release name =
  artifactId, namespace `sdjpa-intro`.
